...
この資料にLP全文、取引条件の表示案、広告案、確認状況、PC・スマートフォンの画面画像を収録しています。レビュー用で、本番公開・広告配信は未実施です。

- [LPのブラウザ表示](https://review.project-mentoring.pages.dev/)
...
本番公開・広告配信は未実施です。この共有ページはレビュー専用で、フォーム送信とアクセス解析を停止しています。
...
確認状況
・実際の受信確認：公開前のローカルフォームから既存のメール受信経路へ合成データを送信し、到着・受付番号・Reply-Toを照合。
・模擬確認：PC／スマホ表示、解析への同意・拒否、送信失敗後の再送、解析が使えないときのフォーム動作。
・未確認：本番ドメインからの実送信、GA4への実到着。技術検査を法的適合確認として扱っていない。

公開・募集前に残る事項
・実際の事業拠点住所・連絡先を請求者へ開示できる準備の確認。
・未利用振替の失効・返金不可条件と、中途解約との関係の法的確認。
・本番ドメインのDNS設定とファイル配置。
電話勧誘販売・特定継続的役務の該当性、必要な法定書面・電子交付は、勧誘・有料契約までの確認事項として残っている。
